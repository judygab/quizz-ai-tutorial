  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        neo: cn(neoClasses, "bg-primary text-primary-foreground border-blue-900 rounded-full"),
      ...
      size: {
        ...
        lg: "rounded-full px-3.5 py-5 h-14",
...
export type SpanProps = VariantProps<typeof span>;
export const span = cva(["absolute", "h-14", "bottom-[-7px]", "w-full", "border-2", "left-0", "z-0"], {
  variants: {
    variant: {
      default: "hidden",
      neo: "border-blue-900 bg-primary-shadow",
      neoOutline: "border-[#6366F6] bg-[#6366a7]",
  ...
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      ...
      <Comp
        ...
        {...props}
      />
        <span className={cx(span({ variant }), borderRound)}></span>
      </div>
    )
  }
)
